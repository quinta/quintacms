<?php
     ...
     * listing a collection of OrderChange objects.  It includes
     ...
     * 
     */
    class OrderChangeDataGridGen extends QDataGrid {
        /**
         ...
         * way of adding a column to this OrderChange datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the OrderChange object, or it can be any QQNode extending from QQN::OrderChange().
         * 
         ...
         * sorting using two fields from the OrderChange object.
         *
         ...
         *
         * @param mixed $mixContent string or QQNode from OrderChange
        ...
        public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
            switch ($intArgumentType) {
                case QMetaControlArgumentType::QueryString:
                    $strLinkUrl .= '?intOrderId=<?=urlencode($_ITEM->OrderId)?>&strDate=<?=urlencode($_ITEM->Date)?>';
                    break;
                case QMetaControlArgumentType::PathInfo:
                    $strLinkUrl .= '/<?=urlencode($_ITEM->OrderId)?>/<?=urlencode($_ITEM->Date)?>';
                    break;
                default:
        ...
        public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
            $strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->OrderId . "," . $_ITEM->Date, false); ?>>' . $strLinkHtml . '</a>';
            $colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
            $this->AddColumn($colEditColumn);
            return $colEditColumn;
        }
                                                                            ...
                                                                            $aryOverrideParameters = null)
        {
            ...
            $strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->OrderId . "," . $_ITEM->Date, false); ?>>' . $strLinkText . '</a>';
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = OrderChange::CountAll();
            }
            ...
            // Set the DataSource to be a Query result from OrderChange, given the clauses above
            $this->DataSource = OrderChange::LoadAll($objClauses);
        }
         ...
         *
         * Given a QQNode or a Text String, this will return a OrderChange-based QQNode.
         * It will also verify that it is a proper OrderChange-based QQNode, and will throw an exception otherwise.
         *
        ...
        protected function ResolveContentItem($mixContent) {
            if ($mixContent instanceof QQNode) {
                ...
                if ($mixContent->_RootTableName == 'order_change') {
                    ...
                    throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "order_change".');
            } else if (is_string($mixContent)) switch ($mixContent) {
                case 'OrderId': return QQN::OrderChange()->OrderId;
                case 'Order': return QQN::OrderChange()->Order;
                case 'Date': return QQN::OrderChange()->Date;
                case 'Notes': return QQN::OrderChange()->Notes;
                case 'Value': return QQN::OrderChange()->Value;
                case 'TypeId': return QQN::OrderChange()->TypeId;
                default: throw new QCallerException('Simple Property not found in OrderChangeDataGrid content: ' . $mixContent);
