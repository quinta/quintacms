<?php
     ...
     * listing a collection of TaxRate objects.  It includes
     ...
     * 
     */
    class TaxRateDataGridGen extends QDataGrid {
        /**
         ...
         * way of adding a column to this TaxRate datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the TaxRate object, or it can be any QQNode extending from QQN::TaxRate().
         * 
         ...
         * sorting using two fields from the TaxRate object.
         *
         ...
         *
         * @param mixed $mixContent string or QQNode from TaxRate
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = TaxRate::CountAll();
            }
            ...
            // Set the DataSource to be a Query result from TaxRate, given the clauses above
            $this->DataSource = TaxRate::LoadAll($objClauses);
        }
         ...
         *
         * Given a QQNode or a Text String, this will return a TaxRate-based QQNode.
         * It will also verify that it is a proper TaxRate-based QQNode, and will throw an exception otherwise.
         *
        ...
        protected function ResolveContentItem($mixContent) {
            if ($mixContent instanceof QQNode) {
                ...
                if ($mixContent->_RootTableName == 'tax_rate') {
                    ...
                    throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tax_rate".');
            } else if (is_string($mixContent)) switch ($mixContent) {
                case 'Id': return QQN::TaxRate()->Id;
                case 'ZoneId': return QQN::TaxRate()->ZoneId;
                case 'Rate': return QQN::TaxRate()->Rate;
                default: throw new QCallerException('Simple Property not found in TaxRateDataGrid content: ' . $mixContent);
