<?php
     ...
     * listing a collection of OrderItem objects.  It includes
     ...
     * 
     */
    class OrderItemDataGridGen extends QDataGrid {
        /**
         ...
         * way of adding a column to this OrderItem datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the OrderItem object, or it can be any QQNode extending from QQN::OrderItem().
         * 
         ...
         * sorting using two fields from the OrderItem object.
         *
         ...
         *
         * @param mixed $mixContent string or QQNode from OrderItem
        ...
        public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
            switch ($intArgumentType) {
                case QMetaControlArgumentType::QueryString:
                    $strLinkUrl .= '?intOrderId=<?=urlencode($_ITEM->OrderId)?>&intProductId=<?=urlencode($_ITEM->ProductId)?>';
                    break;
                case QMetaControlArgumentType::PathInfo:
                    $strLinkUrl .= '/<?=urlencode($_ITEM->OrderId)?>/<?=urlencode($_ITEM->ProductId)?>';
                    break;
                default:
        ...
        public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
            $strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->OrderId . "," . $_ITEM->ProductId, false); ?>>' . $strLinkHtml . '</a>';
            $colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
            $this->AddColumn($colEditColumn);
            return $colEditColumn;
        }
                                                                            ...
                                                                            $aryOverrideParameters = null)
        {
            ...
            $strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->OrderId . "," . $_ITEM->ProductId, false); ?>>' . $strLinkText . '</a>';
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = OrderItem::CountAll();
            }
            ...
            // Set the DataSource to be a Query result from OrderItem, given the clauses above
            $this->DataSource = OrderItem::LoadAll($objClauses);
        }
         ...
         *
         * Given a QQNode or a Text String, this will return a OrderItem-based QQNode.
         * It will also verify that it is a proper OrderItem-based QQNode, and will throw an exception otherwise.
         *
        ...
        protected function ResolveContentItem($mixContent) {
            if ($mixContent instanceof QQNode) {
                ...
                if ($mixContent->_RootTableName == 'order_item') {
                    ...
                    throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "order_item".');
            } else if (is_string($mixContent)) switch ($mixContent) {
                case 'OrderId': return QQN::OrderItem()->OrderId;
                case 'Order': return QQN::OrderItem()->Order;
                case 'ProductId': return QQN::OrderItem()->ProductId;
                case 'Product': return QQN::OrderItem()->Product;
                case 'Quantity': return QQN::OrderItem()->Quantity;
                case 'StatusId': return QQN::OrderItem()->StatusId;
                default: throw new QCallerException('Simple Property not found in OrderItemDataGrid content: ' . $mixContent);
