<?php
     ...
     * listing a collection of Account objects.  It includes
     ...
     * 
     */
    class AccountDataGridGen extends QDataGrid {
        /**
         ...
         * way of adding a column to this Account datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the Account object, or it can be any QQNode extending from QQN::Account().
         * 
         ...
         * sorting using two fields from the Account object.
         *
         ...
         *
         * @param mixed $mixContent string or QQNode from Account
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = Account::CountAll();
            }
            ...
            // Set the DataSource to be a Query result from Account, given the clauses above
            $this->DataSource = Account::LoadAll($objClauses);
        }
         ...
         *
         * Given a QQNode or a Text String, this will return a Account-based QQNode.
         * It will also verify that it is a proper Account-based QQNode, and will throw an exception otherwise.
         *
        ...
        protected function ResolveContentItem($mixContent) {
            if ($mixContent instanceof QQNode) {
                ...
                if ($mixContent->_RootTableName == 'account') {
                    ...
                    throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "account".');
            } else if (is_string($mixContent)) switch ($mixContent) {
                case 'Id': return QQN::Account()->Id;
                case 'RegistrationDate': return QQN::Account()->RegistrationDate;
                case 'Username': return QQN::Account()->Username;
                case 'Password': return QQN::Account()->Password;
                case 'Notes': return QQN::Account()->Notes;
                case 'LastLogin': return QQN::Account()->LastLogin;
                case 'LoginCount': return QQN::Account()->LoginCount;
                case 'Online': return QQN::Account()->Online;
                case 'OnetimePassword': return QQN::Account()->OnetimePassword;
                case 'ValidPassword': return QQN::Account()->ValidPassword;
                case 'TypeId': return QQN::Account()->TypeId;
                case 'StatusId': return QQN::Account()->StatusId;
                case 'PersonId': return QQN::Account()->PersonId;
                case 'Person': return QQN::Account()->Person;
                default: throw new QCallerException('Simple Property not found in AccountDataGrid content: ' . $mixContent);
