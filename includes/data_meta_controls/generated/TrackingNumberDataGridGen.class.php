<?php
     ...
     * listing a collection of TrackingNumber objects.  It includes
     ...
     * 
     */
    class TrackingNumberDataGridGen extends QDataGrid {
        /**
         ...
         * way of adding a column to this TrackingNumber datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the TrackingNumber object, or it can be any QQNode extending from QQN::TrackingNumber().
         * 
         ...
         * sorting using two fields from the TrackingNumber object.
         *
         ...
         *
         * @param mixed $mixContent string or QQNode from TrackingNumber
        ...
        public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
            switch ($intArgumentType) {
                case QMetaControlArgumentType::QueryString:
                    $strLinkUrl .= '?intOrderId=<?=urlencode($_ITEM->OrderId)?>&strNumber=<?=urlencode($_ITEM->Number)?>';
                    break;
                case QMetaControlArgumentType::PathInfo:
                    $strLinkUrl .= '/<?=urlencode($_ITEM->OrderId)?>/<?=urlencode($_ITEM->Number)?>';
                    break;
                default:
        ...
        public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
            $strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->OrderId . "," . $_ITEM->Number, false); ?>>' . $strLinkHtml . '</a>';
            $colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
            $this->AddColumn($colEditColumn);
            return $colEditColumn;
        }
                                                                            ...
                                                                            $aryOverrideParameters = null)
        {
            ...
            $strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->OrderId . "," . $_ITEM->Number, false); ?>>' . $strLinkText . '</a>';
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = TrackingNumber::CountAll();
            }
            ...
            // Set the DataSource to be a Query result from TrackingNumber, given the clauses above
            $this->DataSource = TrackingNumber::LoadAll($objClauses);
        }
         ...
         *
         * Given a QQNode or a Text String, this will return a TrackingNumber-based QQNode.
         * It will also verify that it is a proper TrackingNumber-based QQNode, and will throw an exception otherwise.
         *
        ...
        protected function ResolveContentItem($mixContent) {
            if ($mixContent instanceof QQNode) {
                ...
                if ($mixContent->_RootTableName == 'tracking_number') {
                    ...
                    throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tracking_number".');
            } else if (is_string($mixContent)) switch ($mixContent) {
                case 'OrderId': return QQN::TrackingNumber()->OrderId;
                case 'Order': return QQN::TrackingNumber()->Order;
                case 'Number': return QQN::TrackingNumber()->Number;
                default: throw new QCallerException('Simple Property not found in TrackingNumberDataGrid content: ' . $mixContent);
