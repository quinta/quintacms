...
{
	print '<div class="heading">' . Quinta::Translate('Billing Address') . ': </div>';
	
	$_CONTROL->objAddressSelectionModule->Render();
	
	print '<div class="spacer"></div><hr> <div class="spacer"></div>';
	
	print '<div class="heading">' . Quinta::Translate('Payment Method') . ': </div>';
	...
	if( $_CONTROL->ShowCCInput )
	{
		...
		foreach($_CONTROL->aryPaymentMethodControllers as $objMethodView)
		{
		...
		<table class="CCInput">
			...
			<tr>
				<td><div class="renderWithName"><?php print Quinta::Translate('Expiration Date') . ': '; ?></div></td>
				<td>
					<?php
