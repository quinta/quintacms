...
class InstallationForm extends QForm {
	...
	protected function Form_Create() {
		...
		$this->txtDatabaseAdapter->Text = 'MySqli5';
	...
	protected function btnSaveConfig_Click($strFormId, $strControlId, $strParameter) {
		...
		$this->lblMessage->Text =
			...
			. __QUINTA_WWWROOT__ . '/install* </code></pre></p>';
		$this->blnFinished = true;
	}

	protected function installDatabase() {
		if ($this->chkInstallExampleData->Checked)
			$strSqlFile = __QUINTA_DBDIR__ . '/quinta-with-data.sql';
		else
			$strSqlFile = __QUINTA_DBDIR__ . '/quinta.sql';
		$strCommand = 'mysql -u ' . $this->txtDatabaseUser->Text . ' -h ' . $this->txtDatabaseServer->Text;
		if($this->txtDatabasePassword->Text )
			$strCommand .= ' -p' . $this->txtDatabasePassword->Text;
		$strCommand .= ' ' . $this->txtDatabaseName->Text . ' < ' . $strSqlFile;
		$aryErrors = array();
		$intRetval = 0;
	...
	protected function createDatabase() {
		$strCommand = 'mysql -u ' . $this->txtDatabaseAdminUser->Text . ' -h ' . $this->txtDatabaseServer->Text;
		if($this->txtDatabaseAdminPassword->Text )
	...
	protected function writeConfigFile($strOutFile) {
		$strInFile = $this->strQCodoConfigFile . '-quinta-dist';
		$aryValues = array(
